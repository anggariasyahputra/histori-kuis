...
class QuestionController extends Controller
{
    ...
    public function store(StoreRequest $request)
    {
        ...
        try {
            if (!empty($validated['chapter_id'])) {
                $chapter = Chapter::where('subject_id', $question_data['subject_id'])
            ...
            if ($request->hasFile('video')) {
                ...
                $fileName = 'question-video-' . Str::uuid() . '.' . $video->getClientOriginalExtension();
                $question_data['video_path'] = '/uploads/' . $fileName;
                $video->storeAs('', $fileName, 'upload');
            }
    ...
    public function update(UpdateRequest $request, string $id)
    {
        ...
        try {
            ...
            if ($request->hasFile('video')) {
                if ($target_question->video_path) {
                    Storage::disk('upload')->delete(basename($target_question->video_path));
                }
                $video = $request->file('video');
                $fileName = 'question-video-' . Str::uuid() . '.' . $video->getClientOriginalExtension();
                $question_data['video_path'] = '/uploads/' . $fileName;
            ...
            if (!empty($validated['chapter_id'])) {
                $chapter = Chapter::where('subject_id', $target_question->subject_id)
                    ->where('id', $question_data['chapter_id'])
                    ->first();
                if (!$chapter) {
                    return Reply::error(trans('app.errors.404'), 404);
    ...
    public function destroy(string $id)
    {
        ...
        try {
            ...
            if ($question->video_path) {
                Storage::disk('upload')->delete(basename($question->video_path));
            }
            $question->delete();
            DB::commit();
