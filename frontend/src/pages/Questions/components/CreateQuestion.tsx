...
export default function CreateQuestion({
    ...
    const [options, setOptions] = useState<Option[]>(new Array(4).fill(null).map((_, index) => {
            ...
            <div className={
                ...
                <div className={globalStyles.formContent}>
                    <form onSubmit={(e: SyntheticEvent<HTMLFormElement, SubmitEvent>) => {
                        ...
                        <div className={css(globalStyles.groupInputs, styles.questionInputs)}>
                            ...
                            <div className={globalStyles.wrapItem}>
                                ...
                                <CustomSelect
                                    name='level'
                                    defaultOption={{ label: language?.questionLevel.easy, value: 'easy' }}
                                    options={language ? (['easy', 'medium', 'hard'] as const).map(item => ({
                                        value: item,
                            ...
                            <div className={globalStyles.wrapItem}>
                                <label htmlFor='video'>Video soal (opsional)</label>
                                <input id='video' name='video' accept='video/mp4,video/webm,video/ogg' type='file' />
                                <small>MP4, WEBM, atau OGG. Maksimal 50 MB.</small>
                            </div>
                            <div
                                className={css(appStyles.actionBar, styles.questionActionBar)}>
                                {
                                    <div
                            ...
                            {options.map((option, index) => {
                                return (
                                            <div
                                        key={option.key}
                                                className={css(styles.textareaGroup, globalStyles.wrapItem, globalStyles.textarea, option.key === trueOptionKey ? styles.correctOption : '')}>
                                        <div className={styles.wrapLabel}>
                                                    <label className={styles.answerChoice}>
                                                        <input type='radio' name='correct_answer' checked={option.key === trueOptionKey} onChange={() => setTrueOptionKey(option.key)} />
                                                        <span>{`${language?.answer} ${index + 1}`}</span>
                                                    </label>
                                                    {option.key === trueOptionKey ? <strong className={styles.correctLabel}>Jawaban benar</strong> : null}
                                        </div>
                                        <TextEditor
                                            name='options[]'
                                            defaultContent=''
