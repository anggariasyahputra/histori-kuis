...
export default function Exam() {
    ...
    const [hasReadMaterials, setHasReadMaterials] = useState(false);
    ...
    const materials = queryData.data?.materials || [];
    const materialReadKey = `histori-kuis-material-read-${id}`;
    useEffect(() => {
        if (materials.length === 0) {
            setHasReadMaterials(true);
            return;
        }
        setHasReadMaterials(localStorage.getItem(materialReadKey) === 'true');
    }, [materialReadKey, materials.length]);
                        ...
                        <>
                                                ...
                                                permissions.has('exam_submit')
                                                    ...
                                                    <>
                                                        <Link
                                                            ...
                                                            to='take'
                                                            onClick={e => {
                                                                if (!hasReadMaterials) e.preventDefault();
                                                            }}
                                                            aria-disabled={!hasReadMaterials}
                                                            title={!hasReadMaterials ? 'Baca materi dan centang konfirmasi terlebih dahulu' : undefined}
                                                            type='button'
                                                            className={css(appStyles.actionItem, !hasReadMaterials ? styles.disabledAction : '')}
                                                        >
                            ...
                            {isStudent && materials.length > 0 ?
                                <section className={styles.materialContainer}>
                                    <div className={styles.header}>
                                        <h2 className={styles.title}>Materi sebelum kuis</h2>
                                    </div>
                                    <p className={styles.materialIntro}>Baca materi berikut sebelum mengerjakan kuis.</p>
                                    <div className={styles.materialList}>
                                        {materials.map(material => (
                                            <div className={styles.materialItem} key={material.id}>
                                                <strong>{material.title}</strong>
                                                {material.type === 'video' ?
                                                    <video controls src={material.fileUrl} /> :
                                                    <a href={material.fileUrl} target='_blank' rel='noreferrer'>Buka materi PDF</a>
                                                }
                                            </div>
                                        ))}
                                    </div>
                                    <label className={styles.materialCheck}>
                                        <input
                                            type='checkbox'
                                            checked={hasReadMaterials}
                                            onChange={e => {
                                                setHasReadMaterials(e.currentTarget.checked);
                                                localStorage.setItem(materialReadKey, String(e.currentTarget.checked));
                                            }}
                                        />
