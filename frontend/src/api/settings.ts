...
export async function apiGetBranding() {
    try {
        const res = await request.get(`${prefix}/branding`);
        const { data } = res.data as ApiResponseWithData<{ schoolLogo: string | null; }>;
        return data;
    } catch (error: any) {
...
export async function apiUploadSchoolLogo(formData: FormData) {
    try {
        const res = await request.post(`${prefix}/logo`, formData);
        const { data } = res.data as ApiResponseWithData<{ schoolLogo: string; }>;
        return data;
    } catch (error: any) {
        return apiUtils.handleError(error);
    }
